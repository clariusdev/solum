    ...
    init() {
        ...
        NotificationCenter.default.addObserver(forName: .deviceFound, object: nil, queue: nil) { [weak self] notification in
            ...
            if let row = self.scanners.firstIndex(where: {$0.serial == deviceFound.serial}) {
                ...
                self.scanners.append(Scanner(serial: deviceFound.serial, model: "-", ssid: "-", password: "password", ip: "-", tcpPort: 0, rssi: deviceFound.rssi, battery: deviceFound.battery, temperature: deviceFound.temperature, powered: deviceFound.powered, availability: deviceFound.availability, listenPolicy: deviceFound.listenPolicy, chargingStatus: deviceFound.chargingStatus))
            }
        }
        // Listen for notifications from the cloud model about found scanners
        NotificationCenter.default.addObserver(forName: .scannerCloud, object: nil, queue: nil) { [weak self] notification in
            guard let self = self else {
                return
            }
            guard let userInfo = notification.userInfo else {
            ...
            guard let scannerCloud = userInfo["scannerCloud"] as? ScannerCloud else {
                return
            }
            if let row = self.scanners.firstIndex(where: {$0.serial == scannerCloud.serial}) {
                self.scanners[row].model = scannerCloud.model
                self.scanners[row].certificate = scannerCloud.certificate
            }
        }
